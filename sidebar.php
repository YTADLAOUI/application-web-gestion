    <div class="sidebar bg-white position-relative" style="padding :20px;height:130vh;">
            <div class=" h4  position-relative"  style ="height:10vh;">
                    <a href="principal.php"class="text-decoration-none text-dark">
                        <span class="navbar-logo text-danger "><i class="bi bi-music-note-list"></i></span> 
                                 <b class="name-logo ss">RockStars</b>
                    </a>
            </div>
                        <ul style="font-size:10px;">
                            <li>
                                <a class="active d-flex align-items-center text-dark p-1 ss" style="border-radius 6px;" href="principal.php">
                                    <i class="bi bi-graph-down me-2"></i>
                                    <span class="ss">Dashbord</span>
                                </a>
                            </li>
                            <li>
                                <a class=" d-flex align-items-center  text-dark p-1" style="border-radius 6px;" href="principal.php">
                                    <i class="bi bi-gear-fill me-2"></i>
                                    <span class="ss">Settings</span>
                                </a>
                            </li>
                            <li>
                                <a class=" d-flex align-items-center  text-dark p-1" style="border-radius 6px;" href="principal.php">
                                    <i class="bi bi-person-circle me-2"></i>
                                    <span class="ss">Profile</span>
                                </a>
                            </li>
                            <li>
                                <a class=" d-flex align-items-center text-dark p-1" style="border-radius 6px;" href="instrument.php">
                                    <i class="bi bi-music-note me-2"></i>
                                    <span class="ss">Les instruments</span>
                                </a>
                            </li>
                            <li>
                                <a class=" d-flex align-items-center text-dark p-1" style="border-radius 6px;" href="creation.php">
                                    <i class="bi bi-plus-circle-dotted me-2 h6"></i>
                                    <span class="ss pb-1">add instrument</span>
                                </a>
                            </li>
                        </ul>
                        <ul style=";font-size:10px;"  class="d-flex align-items-end">
                                <a class="d-flex align-items-end text-dark p-1 " style="border-radius 6px;" href="index.php">
                                    <i class="bi bi-box-arrow-left me-2"></i>
                                    <span class="ss">Log out</span>
                                </a>

                        </ul>
                   
    </div>